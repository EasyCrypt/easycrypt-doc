...
of \emph{(real) discrete sub-distribution}. A discrete sub-distribution over
a type $t$ is fully defined by its mass function, i.e. by non-negative function
from $t$ to $\mathbb{R}$ s.t. $\sum_{x \in E} f(x) \le 1$ --- implying that
$f$ has a discrete support. When the sum is equal to $1$, we say that we have a
\emph{distribution}.
%
...
actual type, as in \ec{int distr} or \ec{bool distr}.

...
\ec{OR}, then the module \ec{Or} would still have the procedure
\ec{guessed}. Furthermore, when declaring a module, we can ask \EasyCrypt\ to check whether it satisfies multiple module types, as in:
\begin{easycrypt}{}{}
...
One may also declare \ec{P0} as simply
\begin{easycrypt}{}{}
pred P0.
\end{easycrypt}
...
\ec{&$m$} is a memory and $x$ is a program variable that's in
\ec{&$m$}'s domain, then the extended expression for the value of $x$
in \ec{&$m$}.  Quantification over memories is allowed:
\begin{easycrypt}{}{}
forall &#$m$#, #$\phi$#
\end{easycrypt}
...
\begin{itemize}
\item the local variables and parameters of a
  procedure; and
...
\phl and \prhl judgements, and lossless assertions. When we speak of
running the body of a procedure from an initial memory whose domain is
the global variables of declared modules plus the procedure's
parameters and local variables, we understand that if this process
terminates, the resulting memory will have a component \ec{res}
recording the procedure's return value (it will also contain the final
values of the global variables, as well as the procedure's parameters
and local variables).
...
the judgement) whose names conflict with parameters and local
variables of \ec{$M$.$p$} will refer to the bound identifiers, not the
parameters/local variables.

The informal meaning of the \hl judgement is that, for all initial
...
outside the judgement) whose names conflict with parameters or local
variables of \ec{$M$.$p$} will refer to the bound identifiers, not the
parameters/local variables.

The informal meaning of the \phl judgement is that, for all initial
memories \ec{&$m$} satisfying $\phi$ and whose domains consist of the
global variables of declared modules plus the parameters and local
...
judgement) whose names conflict with parameters and local variables of
...
a bound memory (outside the judgement), then all references to \ec{&1}
(resp., \ec{&2}) in $\phi$ and $\psi$ are renamed to use a fresh
memory.

The informal meaning of the \prhl judgement is that, for all initial
...
local variables of \ec{$N$.$q$}, if $\phi$ holds, then there is an
isomorphism from the probability distribution of final memories of
\ec{$M$.$p$} to the probability distribution of the final memories of
\ec{$N$.$q$} that respects $\psi$, i.e., such that for each pair of
memories $(\ec{&}m,\ec{&}n)$, the formula formed by replacing
references to \ec{&1} with \ec{&$m$}, and references to \ec{&2} with
\ec{&$n$} in \phi, holds. \textbf(This isn't quite right!)}

...
guessed, so far. \ec{Or} \emph{satisfies} the specification of the
...
\ec{&$m$} is a memory and $x$ is a program variable that's currently
...
focus of an \EasyCrypt\ proof goal) then \ec{$x$\{$m$\}} is the
extended expression for the value of $x$ in \ec{&$m$}.  Quantification
...
$\phi$ is a formula and \ec{&$m$} is a memory, then
\ec{($\phi$)\{$m$\}} means the formula in which every subterm $u$ of
$\phi$ consisting of a variable or having the form \ec{glob $\,M$} for
a module $M$ is replaced by \ec{$u$\{$m$\}}.
For example,
\begin{easycrypt}{}{}
...
\item \ec{&$m$} is a memory over the global variables of the modules
  the call of \ec{$M$.$p$} depends on.

\item The formula $\phi$ may involve the term \ec{res}, whose
  type is \ec{$M$.$p$}'s return type. Any memory that results
  from running the procedure will have a component named \ec{res}
...
in a final memory \ec{&$m$} satisfying $\phi$, such that
$\phi\{m\}$ holds.

...
ambient logic---which includes judgements of the \hl, \phl and \prhl
logics.
...
for connecting judgements from the other logics
...
The parentheses around \ec{(int * bool)} and \ec{(real -> int)} are
mandatory.
...
\item (\hl Judgments)\quad A \hl \emph{judgement} has the form
  \begin{center}
...
\item (\phl Judgments)\quad A \phl \emph{judgement} has one of the forms
  \begin{center}
...
\item (\prhl Judgments)\quad A \prhl \emph{judgement} has the form
  \begin{center}
...
equivalence like \ec{=\{res\}} (i.e., \ec{res\{1\} = res\{2\}}), this
  is particulary easy to interpret.)
